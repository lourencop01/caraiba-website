    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          ...
          <button
            onClick={closeCart}
            aria-label={ta('closeCart')}
            className="p-2 text-foreground-light hover:text-primary-dark transition-colors rounded-full hover:bg-background cursor-pointer"
          >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {lines.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
              ...
              <p className="text-foreground-light">{t('empty')}</p>
            </div>
          ) : (
            lines.map((line) => (
              <div
